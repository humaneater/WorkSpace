...
public class ModelImporterForOutline : AssetPostprocessor
{
    bool IsHeroSkin(string name)
    {
        return name.StartsWith("HER_") && name.Contains("_Skin");
    }
    
    // 在GameObject生成后调用，对GameObject的修改会影响生成结果，但引用不会保留
    void OnPostprocessModel(GameObject g)
    {
        if (!IsHeroSkin(g.name))
            return;

        ModelImporter model = assetImporter as ModelImporter;

        Dictionary<string, Mesh> originalMeshes = GetMesh(g, mf => mf.sharedMesh, smr => smr.sharedMesh);

        Dictionary<string, Mesh> instantiatedMeshes = GetMesh(g, mf => UnityEngine.Object.Instantiate(mf.sharedMesh), smr => UnityEngine.Object.Instantiate(smr.sharedMesh));
        foreach (var mesh in instantiatedMeshes.Values)
        {
            RecalculateNormals(mesh, 180f);
        }

        foreach (var originalMesh in originalMeshes)
        {
            ComputeSmoothedNormal(instantiatedMeshes[originalMesh.Key], originalMesh.Value);
        }
    ...
    Dictionary<string, Mesh> GetMesh(GameObject go, Func<MeshFilter, Mesh> onIterateMf, Func<SkinnedMeshRenderer, Mesh> onIterateSmr)
    {
        ...
        foreach (var item in go.GetComponentsInChildren<MeshFilter>())
            AddMesh(dic, item.name, onIterateMf(item));

        var mf = go.GetComponent<MeshFilter>();
        if (mf != null)
            AddMesh(dic, mf.name.Replace("@", ""), onIterateMf(mf));

        foreach (var item in go.GetComponentsInChildren<SkinnedMeshRenderer>())
            AddMesh(dic, item.name, onIterateSmr(item));

        var smr = go.GetComponent<SkinnedMeshRenderer>();
        if (smr != null)
            AddMesh(dic, smr.name.Replace("@", ""), onIterateSmr(smr));

        return dic;
    }

    void ComputeSmoothedNormal(Mesh smoothedMesh, Mesh originalMesh, int maxOverlapvertices = 50)
    {
        int svc = smoothedMesh.vertexCount, ovc = originalMesh.vertexCount;
        // CollectNormalJob Data

        Vector3[] smoothNormals = smoothedMesh.normals;
        Vector3[] smoothVertices = smoothedMesh.vertices;

        var averageNormalHash = new Dictionary<Vector3, Vector3>();
        //var averageNormalHash = new Dictionary<Vector3, Vector3>();

        // NormalBakeJob Data
        Vector3[] normalsO = originalMesh.normals;
        Vector3[] vertrxO = originalMesh.vertices;
        Vector4[] tangents = originalMesh.tangents;
        Vector2[] uv8 = new Vector2[ovc];


        for (var j = 0; j < smoothVertices.Length; j++)
        {
            if (!averageNormalHash.ContainsKey(smoothVertices[j]))
            {
                averageNormalHash.Add(smoothVertices[j], smoothNormals[j]);
            }
            else
            {
                averageNormalHash[smoothVertices[j]] = (averageNormalHash[smoothVertices[j]] + smoothNormals[j]).normalized;
            }
        }

        for (int i = 0; i < vertrxO.Length; i++)
        {
            var smoothModelNormal = averageNormalHash[vertrxO[i]];

            var bitangent = (Vector3.Cross(normalsO[i], new Vector3(tangents[i].x, tangents[i].y, tangents[i].z)) * tangents[i].w).normalized;
            ...
            var bakedNormal = math.mul(smoothModelNormal, tbn);
            uv8[i] = bakedNormal.xy;

        }
        originalMesh.uv3 = uv8;
    }
