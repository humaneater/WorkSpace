...
float4 GetScatteringTextureUvwzFromRMuMuSNu(AtmosphereParameter atmosphere,float r, float mu, float mu_s, float nu,bool ray_r_mu_intersects_ground)
{
  // Distance to top atmosphere boundary for a horizontal ray at ground level.
  float H = sqrt(atmosphere.top_radius * atmosphere.top_radius -
      atmosphere.bottom_radius * atmosphere.bottom_radius);
  ...
  if (ray_r_mu_intersects_ground) {
    ...
    float d_max = rho;
    u_mu = 0.5 - 0.5 * GetTextureCoordFromUnitRange(d_max == d_min ? 0.0 :(d - d_min) / (d_max - d_min), SCATTERING_TEXTURE_SIZE.z / 2);
  } else {
...
void GetRMuMuSNuFromScatteringTextureUvwz(AtmosphereParameter atmosphere,float4 uvwz,out float r, out float mu, out float mu_s,out float nu, out int ray_r_mu_intersects_ground)
{
...
void GetRMuMuSNuFromScatteringTextureFragCoord(
    ...
    float4 uvwz =
        float4(frag_coord_nu, frag_coord_mu_s, frag_coord.y, frag_coord.z) / SCATTERING_TEXTURE_SIZE;
