...
namespace PrecomputerScatter
{
    public static class Const
    {
        ...
        //太阳角半径，需要小于0.1，因为该实现中假设角半径很小
        public const double SunAngularRadius = 0.00935 / 2.0;
        public const double SunSolidAngle = Mathf.PI * SunAngularRadius * SunAngularRadius;
        ...
        public static ModelParams DefaultParam()
        {
            ...
            //光波长
            var wavelengths = new List<double>(nLambda);
            //太阳光不同波长的irradiance
            var solarIrradiance = new List<double>(nLambda);
            //不同波长的rayleighScattering
            ...
            for (int l = LambdaMin; l <= LambdaMax; l += DeltaLambda)
            {
                ...
                //rayleigh和mie的计算方式，"Precomputed Atmospheric Scattering" 有提到
                double lambda = (double)l * 1e-3;// micro-meters, 纳米(nm)->微米(um)
                double rayleigh = Rayleigh * Math.Pow(lambda, -4);
                double mie = MieAngstromBeta / MieScaleHeight * Math.Pow(lambda, -MieAngstromAlpha);
                
                wavelengths.Add(l);
                solarIrradiance.Add(SolarIrradiance[index]);
                rayleighScattering.Add(rayleigh);
